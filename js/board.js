$(document).ready(function() {
	...
	const gameToPlay = function(game) {
		if (game === "Tic-Tac-Toe") {
			...
			Swal.mixin({
				// gets user names to play the game and saves the names in variables
				input: "text",
	...
	$(".button").on("click", function() {
		// play again button - restarts game and clears all text pertaining to the previous game
		$("svg").hide();
		$("p").hide();
	...
	$(".score").on("click", function() {
		Swal.fire({
			title: "Score Update",
			text: `${player1.toUpperCase()}: ${
				playerScores[player1]
			} VS ${player2.toUpperCase()}: ${playerScores[player2]}`,
	...
	$(".difficulty").on("click", function() {
		// initial click after difficulty selected
...
const turn = function() {
	//changes the players turn after each go
	move === player1 ? (move = player2) : (move = player1);
};

const win = function() {
	//checks for a win and makes changes accordingly
...
const draw = function() {
	// checks for a draw and configs changes accordingly
...
const hint = function() {
	// should check to see if all input is correct and if so then it gives the player 3 numbers as a hint (can flash them on the screen quickly)

	let inputSoFar = $("input"); //can turn into an array to compare to original
	let stringSudoku = $("input")
		.map((i, element) => ($(element).val() >= 1 ? $(element).val() : "."))
		.get()
		.join(""); //gets a string of all input for comparison
	...
	$(`#cell-${hint1}`)
		.val(solvedBoard[hint1])
		.hide()
		.fadeIn(300)
		.addClass("hint");
	$(`#cell-${hint2}`)
		.val(solvedBoard[hint2])
		.hide()
		.fadeIn(300)
		.addClass("hint");
	$(`#cell-${hint3}`)
		.val(solvedBoard[hint3])
		.hide()
		.fadeIn(300)
		.addClass("hint");

	const sleep = function(ms) {
		// allows the hint to show and then go
...
const solveBoardEffect = function() {
	...
	if (stringSudoku.indexOf(".") >= 0) {
		...
		//check to see if the players input it correct
		if (stringSudoku.join("") === solvedBoard) {
			$(".choose")
				.removeClass("invisible")
				.fadeIn(300)
				.text("Congratulations you are the Sudoku Master!");
		} else {
			...
			$(".choose")
				.removeClass("invisible")
				.fadeIn(300)
				.text("You have an error somewhere. Watch the board for a hint.");
		}
	}
};
